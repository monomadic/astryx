...
use interpreter::SiteBuilder;
pub use models::{Object, Site, State};

/// Compiles program text into an output graph
pub fn _parse_from_string(
    input: &str,
    path: &str,
    state: Option<State>,
) -> AstryxResult<SiteBuilder> {
    ...
    parser::parse(lines)
        .and_then(|statements| interpreter::_run(&statements, state))
        .map(|objects| objects.into())
}

/// Compiles a source file into an output graph
pub fn _parse_from_file<P: AsRef<Path>>(
    input: P,
    state: Option<State>,
) -> AstryxResult<SiteBuilder> {
    // if no initial state is given to us, pass new empty state.
    ...
    _parse_from_string(&input, &path, Some(state))
}
