...
pub(crate) trait TypeGetters {
    ...
    fn get_required_path(&self, ident: &str) -> AstryxResult<PathBuf>;
    fn get_string(&self, ident: &str) -> Option<String>;
    fn get_path(&self, ident: &str) -> Option<PathBuf>;
}
...
impl TypeGetters for NamedArguments {
    ...
    fn get_path(&self, arg: &str) -> Option<PathBuf> {
        self
            .get_string(arg)
            .map(PathBuf::from)
    }
    ...
    fn get_required_string(&self, ident: &str) -> AstryxResult<String> {
        self.get_string(ident)
            .ok_or(AstryxError::new_from(AstryxErrorKind::MissingRequiredArgument(
                String::from(ident),
            )))
    }
