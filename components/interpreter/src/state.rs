...
impl State {
    ...
    pub(crate) fn resolve(&self, variable: &Variable) -> InterpreterResult<Value> {
        Ok(match variable {
            Variable::QuotedString(s) => Value::String(s.clone()),
            Variable::Reference(r) => {
                self.get(r).ok_or(InterpreterError::UnresolvedReferece)?
                // .map(|v| v.clone())
            ...
            _ => {
                return Err(InterpreterError::UnresolvedReferece);
            }
        })
    }
    ...
    pub(crate) fn interpolate_string(&self, tokens: &Vec<StringToken>) -> InterpreterResult<String> {
        tokens
            .iter()
