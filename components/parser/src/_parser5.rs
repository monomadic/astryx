...
fn function_call_arguments<'a, E: ParseError<&'a str> + ContextError<&'a str>>(i: &'a str) -> IResult<&'a str, &'a str, E> {
    // cut | all_consuming | complete
    cut(all_consuming(alphanumeric1))(i)
...
fn test_function_call_arguments() {
    ...
    let err = function_call_arguments::<VerboseError<Span>>(Span::new(",")).unwrap_err();

    // println!("{:?}", nom::error::convert_error(",", err));
